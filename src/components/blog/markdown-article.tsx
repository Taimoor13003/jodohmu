...
type ArticleQuote = { arabic: string; translation: string; reference: string };
type ArticleSection = { heading: string; paragraphs: string[]; list?: string[]; quotes?: ArticleQuote[]; image?: string; imageAlt?: string };
...
function renderInlineText(text: string) {
  return text.split(/(\*\*.*?\*\*|\[[^\]]+\]\([^\)]+\))/g).map((part, index) => {
    ...
    if (link) {
      const className = "font-bold text-[#263b6b] underline decoration-[#f1a0ba]/70 underline-offset-4 transition hover:text-[#c52d5a]";
      if (link[2].startsWith("/")) return <Link key={index} href={link[2]} className={className}>{link[1]}</Link>;
      return <a key={index} href={link[2]} target="_blank" rel="noopener noreferrer" className={className}>{link[1]}</a>;
    }
    return part;
  });
}
  ...
  return <main className="bg-[#fffdfd] pb-24 text-[#263452]">
    ...
    <article className="container max-w-6xl px-5 pt-14 sm:pt-20">
      <div className="grid gap-12 lg:grid-cols-[220px,minmax(0,1fr)] lg:gap-16">
        ...
        <div>
          <div className="space-y-16">{article.sections.map((section, index) => <section id={`section-${index + 1}`} key={section.heading} className="scroll-mt-24"><h2 className="font-serif text-3xl font-bold leading-tight text-[#11275b] sm:text-4xl">{renderInlineText(section.heading)}</h2><div className="mt-6 space-y-5 text-base leading-8 text-[#53617d] sm:text-lg">{section.paragraphs.map((paragraph) => <p key={paragraph}>{renderInlineText(paragraph)}</p>)}</div>{section.quotes && <div className="mt-8 space-y-5">{section.quotes.map((quote) => <figure key={quote.arabic} className="overflow-hidden rounded-2xl border border-[#ead7a4] bg-[#fffaf0] shadow-[0_10px_28px_rgba(103,77,19,.08)]"><blockquote dir="rtl" lang="ar" className="border-b border-[#f0dfb8] px-6 py-6 text-right font-serif text-2xl leading-[2] text-[#5c4217] sm:px-8 sm:text-3xl">{quote.arabic}</blockquote><figcaption className="px-6 py-5 sm:px-8"><p className="text-base leading-7 text-[#53617d]">{renderInlineText(quote.translation)}</p><p className="mt-3 text-sm font-bold text-[#9B2242]">{renderInlineText(quote.reference)}</p></figcaption></figure>)}</div>}{section.list && <ul className="mt-7 space-y-3 border-l-2 border-[#f1a0ba] pl-5 text-base leading-7 text-[#43516d]">{section.list.map((item) => <li key={item}>{renderInlineText(item)}</li>)}</ul>}{section.image && <Image src={section.image} alt={section.imageAlt ?? ""} width={1536} height={1024} sizes="(max-width: 768px) 100vw, 768px" className="mt-10 aspect-[3/2] w-full rounded-3xl object-cover shadow-[0_18px_50px_rgba(27,52,107,.12)]" />}</section>)}</div>
